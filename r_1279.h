...
#include <cmath>
#include <cstdlib>
#include <unistd.h> //C++ library for getpid() 

/* *************************************************************
        < The R1279 random number generator >
For g++ compiler, you only have to add #include "r_1279.h"
 
The available routines are

r1279		generates a real random number with a uniform
distribution between 0 and 1.

ir1279		generates an integer random number with a uniform
distribution between 0 and 2^31 - 1.

ir1279range(imin, imax)
generates an integer random number from imin through
imax (both integers).

setseed(&iseed)	generates a fairly random integer seed "iseed" from the
time of day and the process ID.

setr1279(iseed)	sets r1279 from the value of iseed (an integer).

Hence to create a random seed and initialize the generator 
 from it put the following lines in your code:
